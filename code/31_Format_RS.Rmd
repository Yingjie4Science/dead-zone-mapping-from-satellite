...
## -> for years 2003-2019, there are 61 files
...
for (yr in yr_ini:yr_end) { ## 2000:2019
  ...
  ### 1.1. loop-terr -----------------------------------------------------------------------
  files <- list.files(path = dir_csv, pattern = paste0('^', yr, '.*', 'terr', '.*\\.csv$'), full.names = T); files
  cat('\nNumber of files:', length(files), 'in', yr)
  ...
  if(yr < 2003){
    ...
    files <- list.files(path = dir_csv, pattern = paste0('^', yr, '.*', 'aqua', '.*\\.csv$'), full.names = T); files
    cat('\nNumber of files:', length(files), 'in', yr) ## (2019-2005+1)*14 = 210
  ...
  ### data3: wind_speed --------------------------------------------------------------------
  files <- list.files(path = dir_csv, pattern = paste0('^', yr, '.*', 'wind_speed', '.*\\.csv$'), full.names = T); files
  cat('\nNumber of files:', length(files), 'in', yr)
  ...
  ds4 <- ds %>%
    dplyr::filter(nchar(band)<14) %>% ## remove data at depth >= 100 if any
    dplyr::select(-featureID) 
  ...
  ds5 <- ds %>%
    dplyr::filter(nchar(band)<12) %>%    ## remove data at depth >= 100 if any
    dplyr::select(-featureID) 
