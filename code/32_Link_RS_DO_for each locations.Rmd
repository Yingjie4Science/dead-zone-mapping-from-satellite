...
df_merge1 <- df_merge %>% 
  dplyr::select(-c("velocity_u_0":"salinity_80"))

df_merge2 <- merge(x = df_merge1, y = df_rs_stra %>% dplyr::select(-n_day_ago), 
                   by = c("YEID", "Date", "date_img", "Depth"))

## 2. to get the column names in the same order as the original data
...
```{r - 3.1 Composite by date range, message=FALSE, warning=FALSE, include=FALSE}
...
for (nw in seq(1, 8)) {
  ...
  unique(df_merge_nw_comp$YEID) %>% length() %>% cat('\ntotal YEID:', .)

  ### 4. write to .xlsx ------------------------------
  prefix <- paste0(interval, 'x', nw, time_scale); prefix 
  ...
  # readr::write_csv(x = cbind(nweek_before = nw, df_merge_nw_comp), path = fname)
  ...
  f_noNA <- df_merge_nw_comp
  f_shp  <- st_as_sf(x = f_noNA, coords = c("lon", "lat"), remove = F, crs = projcrs) ## crs = 4326 (not sure?)
  
  ### write to .shp
  # shp_name <- paste0('sample_2005to2019_pixelValue_with_', whichDO, '_', nw, 'x', prefix, '_noNA', '.shp');  print(shp_name)
  # shp_name <- paste0('RS_', whichDO, '_sample_2000_2019_', nw, 'x', prefix, '.shp');  #print(shp_name)
  ...
  # df_merge_nw_comps <- rbind(df_merge_nw_comps, cbind(nweek_before = nw, df_merge_nw_comp))
}
```
...
```{r - 3.2 Composite by picking the Nearest day to fill NA, include=FALSE}

## --> to use the same settings but aim to limit the date range within (interval * nw) days
interval <- 11
nw <- 1
...
n  <- ncol(df_merge_nw_comp);  n
nm <- names(df_merge_nw_comp); nm
id_ls <- unique(df_merge_nw_comp$YEID)
id_ls %>% length()
...
```{r - 3.3 Composite by day, message=FALSE, warning=FALSE, include=FALSE}
...
for (nw in seq(0, 90)) {
  ...
  ### save as xlsx --------------------------
  fname <- paste0(dir.sample, 'rs_do_sample_lagByDay/', 'RS_', whichDO, '_sample_2000_2019_byday_', nw, 'dayBefore.xlsx'); 
  # print(fname)
  print(basename(fname))
  writexl::write_xlsx(x = cbind(nweek_before = nw, df_merge_nw_comp), path = fname)
