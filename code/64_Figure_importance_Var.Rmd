...
fname <- paste0('./data/results_RF/rf_importance_by_yearNO_26vars.csv') ## all variables
fname <- paste0('./data/results_RF/rf_importance_by_yearNO_18vars.csv') ## remove highly correlated variables, with all data sources
fname <- paste0('./data/results_RF/rf_importance_by_yearNO_18vars_subset_SEAMAP.csv') ## with only SEAMAP data

# fname <- paste0('./data/results_RF/rf_importance_by_yearNO_12vars.csv') ## included in the final model
...
importance_df_summ$group <- factor(x = importance_df_summ$group, 
                                    ...
                                    labels = labels_my[1:length(unique(importance_df_summ$group))])
...
n_var <- length(unique(importance_df_summ$varnames)); n_var
...
f <- paste0(dir.fig, paste('var', f, 'order_0-30', today, sep = '_'), '.png'); f
ggsave(filename = f, plot = last_plot(), width = 6.5, height = fig_h, units = 'in', dpi = 300)
...
ggsave(filename = f, plot = last_plot(), width = 7/2, height = fig_h/2, units = 'in', dpi = 300)
```
