...
importance_df_summ %>%
  ...
  geom_errorbar(aes(ymin=value-ci, ymax=value+ci), width=.5, position=position_dodge(0.05), 
                size = 0.2, alpha = 0.7, color = 'gray50') +
...
f <- paste0(dir.fig, 'var_', f, '_order_0-30.png'); f
ggsave(filename = f, plot = last_plot(), width = 7, height = 6/3, units = 'in', dpi = 500)
...
f <- gsub('rf_|.csv', '', basename(fname))
f <- paste0(dir.fig, 'var_', f, '_order_finalModel.png'); #f
...
var <- rbind(
  readr::read_csv(f1) %>% gather(key = year, value = value, 2:ncol(.)),
  readr::read_csv(f2) %>% gather(key = year, value = value, 2:ncol(.))
)
...
f <- paste0(dir.fig, 'var_sst_chlor_', time_scale, '_median.png'); f
...
tn_ar <- readr::read_csv(f1) 
tn_mr <- readr::read_csv(f2) 
...
f <- paste0(dir.fig, 'var_TN_chlor_', time_scale, '_median2.png'); f
ggsave(filename = f, plot = last_plot(), width = 12/1.5, height = 10/1.5, units = 'in', dpi = 400)
