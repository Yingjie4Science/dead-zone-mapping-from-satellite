...
dir.ctd <- './DO_LUMCON/Rabalais_zip'
setwd(dir.ctd); getwd()
...
ls.gz <- list.files(path = './', pattern = '*.gz', full.names = T)
...
getwd()
# list.dirs(path = './', full.names = T, recursive = T) 
list.files(path = './', pattern = paste0('*', yr, '.*'), full.names = T, recursive = T)

st <- list.files(path = './', pattern = paste0(yr, '_StationList.csv'), full.names = T, 
                 recursive = T, all.files = T, ignore.case = T); st

sb <- list.files(path = './', pattern = paste0(yr, '_Seabird.csv'), full.names = T, 
                 recursive = T, all.files = T, ignore.case = T); sb

ys <- list.files(path = './', pattern = paste0(yr, '_YSI6820.csv'), full.names = T, 
...
st <- list.files(path = './', pattern = paste0(yr, '_StationList.txt'), full.names = T, 
                 recursive = T, all.files = T, ignore.case = T); st

sb <- list.files(path = './', pattern = paste0(yr, '_Seabird.txt'), full.names = T, 
                 recursive = T, all.files = T, ignore.case = T); sb

ys <- list.files(path = './', pattern = paste0(yr, '_YSI6820.txt'), full.names = T, 
                 recursive = T, all.files = T, ignore.case = T); ys
...
sb2 %>% ggplot() +  ## July 25 ~ Aug 5
  geom_bar(aes(x = date)) + 
  scale_x_date(date_breaks = "1 day", date_labels =  "%m-%d") + ## %Y-%m-%d
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ys2 %>% filter(month(date) > 6 & month(date) < 9) %>%
  ggplot() +
  geom_bar(aes(x = date)) + 
...
getwd()
# fname <- paste0('./process/Samping DO by date across station', '.png'); fname
