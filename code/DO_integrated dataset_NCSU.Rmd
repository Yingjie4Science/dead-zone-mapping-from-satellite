...
# save(sb, hh, st, file = paste0(dir.output, "RawData.RData"))
...
### save to csv ---------------------------------------------------------------
today <- format(Sys.time(), "%Y%m%d"); print(today)
fnamerdt <- paste0(dir.output, 'DO_Integrated_Matli_NCSU_cleaned_', today, '.RData'); fnamerdt
fnamecsv <- paste0(dir.output, 'DO_Integrated_Matli_NCSU_cleaned_', today, '.csv');   fnamecsv
# save(df, sb, hh, sp, file = fnamerdt)
...
fname <- "./data_cleaned/DO_Integrated_Matli_NCSU_cleaned_20201104.RData"
load(fname)
```
...
ggplot() + 
  ...
  geom_point(data = sp,  # use geom_point, instead of geom_sf
...
ggplot() + 
  ...
  theme_bw()
pname <- paste0(dir.fig, 'map_samp_by source1', '.png'); pname
ggsave(filename = pname, plot = last_plot(), 
       width = 14, height = 8, units = 'in', dpi = 300)
